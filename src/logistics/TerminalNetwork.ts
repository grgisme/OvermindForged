...
export class TerminalNetwork implements ITerminalNetwork {
	...
	constructor(terminals: StructureTerminal[]) {
		...
		this.notifications = [];
		this.averageFullness = _.sum(this.terminals, t => t.store.getUsedCapacity() / t.storeCapacity) / this.terminals.length;
	}
	...
	private remainingRoomCapacity(room: Room): number {
		...
		if (room.storage) {
			remainingCapacity += room.storage.storeCapacity - room.storage.store.getUsedCapacity();
		}
		if (room.terminal) {
			remainingCapacity += room.terminal.storeCapacity - room.terminal.store.getUsedCapacity();
		}
	...
	private handleExcess(terminal: StructureTerminal, threshold = 25000): void {
		...
		const terminalNearCapacity = terminal.store.getUsedCapacity() > 0.95 * terminal.storeCapacity;
	...
	private equalize(resourceType: ResourceConstant, terminals = this.terminals, verbose = false): void {
		...
		for (const [sender, receiver] of _.zip(senders, receivers)) {
			...
			sendAmount = Math.min(sendAmount, (sender.store[resourceType] || 0) - sendCost - 10,
								  (receiver.storeCapacity - receiver.store.getUsedCapacity()));
			...
			if (state.type == 'out' || state.type == 'in/out') {
				if (terminal.cooldown == 0 && amount > targetAmount + tolerance) {
					...
					if (receiver) {
						...
						if (receiver.storeCapacity - receiver.store.getUsedCapacity() > sendAmount) {
